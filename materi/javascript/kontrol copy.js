...
function mulai() {
  ...
  if (sekolah.value != "0") {
    if (sekolah.value == "1") {
      sekolahfix = "SDN Antasan Kecil Timur 3";
    }
    cek += 1;
  }
...
function next() {
  pilihan1.checked = false;
  pilihan2.checked = false;
  pilihan3.checked = false;
  pilihan4.checked = false;
        ...
        snapshot.forEach(function (childSnapshot) {
          ...
          for (let ii = 0; ii < Namagambar.length; ii++) {
            if (soalid == Namagambar[ii]) {
              ...
              htmls.push(
                '<center><img width="50%" src="' + Gambar + '"></center>'
              );
              $("#gambar").html(htmls);
            } else {
              htmls.push("<a><a/>");
...
function hari() {
  ...
  tanggallengkap =
    namahari[harii] + ", " + tanggal + " " + namabulan[bulan] + " " + tahun;
...
function selesai() {
  ...
  if (jwb != 10) {
    ...
    let hasilinput = document.querySelector(".skor");
    hasilinput.innerText = skor;
